...
class DeliveryBackend:
    ...
    def connect(self):
        """Connect to Delivery radio"""
        print(f"\n[DELIVERY] Connecting to {self.port} @ {self.baud}...")
        try:
            ...
            print(f"[DELIVERY] ✅ Connected to {self.port}")
            self.connected = True
            return True
    ...
    async def websocket_handler(self, websocket):
        ...
        print(f"[DELIVERY_WS] Client connected. Total: {len(self.ws_clients)}")
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    if data.get("action") == "command":
                        ...
                        print(f"[DELIVERY_WS] Command: {cmd}")
            ...
            print(f"[DELIVERY_WS] Client disconnected. Total: {len(self.ws_clients)}")
    ...
    def process_data(self, data: str):
        ...
        if "PONG" in data or "OK" in data:
            print(f"  [DELIVERY] ✅ {data}")
        elif "ERROR" in data:
            print(f"  [DELIVERY] ❌ {data}")
        elif "ARMED" in data or "DISARMED" in data:
            print(f"  [DELIVERY] ⚠️  {data}")
        else:
            print(f"  [DELIVERY] >> {data}")
    ...
    def run_interactive(self):
        ...
        print("\n" + "="*60)
        print("  DELIVERY DRONE TERMINAL")
        print("="*60)
        print("Commands: PING, ARM, TAKEOFF:X, GOTO:lat,lon,alt, RTL, QUIT")
        print("="*60 + "\n")
        ...
        print("\n[INFO] Shutting down Delivery backend...")
        self.running = False
